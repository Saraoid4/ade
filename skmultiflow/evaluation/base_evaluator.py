...
from skmultiflow.metrics import WindowClassificationMeasurements, ClassificationMeasurements, MultiTargetClassificationMeasurements, WindowMultiTargetClassificationMeasurements, RegressionMeasurements, WindowRegressionMeasurements, MultiTargetRegressionMeasurements, WindowMultiTargetRegressionMeasurements, RunningTimeMeasurements
import skmultiflow.utils.constants as constants
from skmultiflow.utils.utils import calculate_object_size
from diversity.diversity_measures import DisagreementEvaluator
...
class StreamEvaluator(BaseObject, metaclass=ABCMeta):
	...
	def _init_metrics(self):
		...
		# Diversity
		self.diversity_eval_measurements = [[], []]
		self.diversity_eval_measurements[0].append(DisagreementEvaluator(model_list=self.model[1:self.n_models], alpha=0.995))
		self.diversity_eval_measurements[1].append(DisagreementEvaluator(model_list=self.model[1:self.n_models], alpha=0.995))
		...
		for metric in self.metrics:
			...
			elif metric == constants.DIVERSITY:
				data_ids = ['disagreement', 'disagreement_ens']
	...
	def _update_metrics(self):
		...
		for metric in self.metrics:
			...
			if metric == constants.ACCURACY:
				...
				values = [[], []]
				values[0].append(self.diversity_eval_measurements[0][0].get_disagreement(self.n_models - 1))
				values[1].append(self.diversity_eval_measurements[1][0].get_disagreement_ens(self.n_models - 1))
			...
			if metric == constants.TRUE_VS_PREDICTED:
				...
				self._data_buffer.update_data(sample_id=sample_id, metric_id=metric, data_id='disagreement',
											  value=values[0])

				self._data_buffer.update_data(sample_id=sample_id, metric_id=metric, data_id='disagreement_ens',
											  value=values[1])
				'''self._data_buffer.update_data(sample_id=sample_id, metric_id=metric, data_id='doublefault',
											  value=values[1])
				self._data_buffer.update_data(sample_id=sample_id, metric_id=metric, data_id='correlation',
											  value=values[2])'''
			...
			with open(self.output_file, 'w+') as f:
				...
				for metric in self.metrics:
					if metric == constants.ACCURACY:
						...
						header += ',mean_disagreement_[{0}]],mean_disagreement_ens_[{0}]]'.format("diversity")
					elif metric == constants.DRIFT:
						header += ',ph_corr_[{0}],ph_mse_[{0}],ph_mae_[{0}]'.format('drift')
					else:
						for i in range(self.n_models):
		...
		if self.output_file is not None:
			...
			for metric in self.metrics:
				if metric == constants.TRUE_VS_PREDICTED:
					...
					disagreement = self._data_buffer.get_data(metric_id=metric, data_id='disagreement')
					disagreement_ens = self._data_buffer.get_data(metric_id=metric, data_id='disagreement_ens')
					#df = self._data_buffer.get_data(metric_id=metric, data_id='doublefault')
					#corr = self._data_buffer.get_data(metric_id=metric, data_id='correlation')
					values = (disagreement, disagreement_ens)
					line += ',{:.6f},{:.6f}'.format(values[0][0], values[1][0])

				elif metric == constants.DRIFT:
					ph_corr = self._data_buffer.get_data(metric_id=metric, data_id='corr')
					ph_mse = self._data_buffer.get_data(metric_id=metric, data_id='mse')
					ph_mae = self._data_buffer.get_data(metric_id=metric, data_id='mae')
					values = (ph_corr, ph_mse, ph_mae)
					line += ',{:.6f},{:.6f},{:.6f}'.format(values[0][0], values[1][0], values[2][0])
