        ...
        while ((self.global_sample_count < actual_max_samples) & (self._end_time - self._start_time < self.max_time)
            ...
            try:
                ...
                if X is not None and y is not None:
                    ...
                    #    self.diversity_eval_measurements[0][0].add(np.array(prediction[1:]).T[0],  list(range(self.n_models - 1)), y)
                    #    self.diversity_eval_measurements[1][0].add_ens(np.array(prediction[1:]).T[0], list(range(self.n_models-1)), prediction[0])
