...
class PageHinkley(BaseDriftDetector):
    ...
    """
    def __init__(self, min_num_instances=30, delta=0.005, _lambda=50, alpha=1-0.0001, increase=False, decrease=False):
        super().__init__()
    ...
    def add_element(self, x):
        ...
        if self.decrease:
            self.max = max(self.max, self.sum)
        if self.increase:
        ...
        if self.sample_count < self.min_instances:
            return None
        if self.decrease:
            if self.max-self.sum > self._lambda:
                self.in_concept_change = True
        if self.increase:
            if self.sum - self.min > self._lambda:
                self.in_concept_change = True
