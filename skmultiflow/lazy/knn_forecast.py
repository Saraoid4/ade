...
class KNNForecast(StreamModel):
    ...
    def __init__(self, n_neighbors=5, max_window_size=1000, leaf_size=30, categorical_list=None, weighted=False):
        super().__init__()
        ...
        self.first_fit = True
        # TODO: Change classes into different target values
        self.classes = []
    ...
    def predict_proba(self, X):
        ...
        for i in range(r):
            ...
            values = [self.window.get_targets_matrix()[index][0] for index in new_ind[i]]
            dist = new_dist[i]
            if self.weighted:
                weights = [np.exp(-d) for d in dist]
            else:
                weights = None

            proba.append(np.average(values, weights=weights))
